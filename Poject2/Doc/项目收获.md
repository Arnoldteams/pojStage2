...
使用了什么技术，使用的场景，为什么用，用了有什么优势，有什么可以改进的地方，不需要说明使用细节（怎么使用可以另附文档）
...
在和数据库作数据交换的时候，会出现数组和字符串的转换。

由于数据库无法进行数组的存储，需要通过类型转换获得对应的数据类型

### AOP

AspectJ

对某些特定的方法进行方法增强。对于切入点使用了execution和@annotation两种，

- @annotation使用了自定义注解，在需要增强的方法上添加该自定义注解，即可对该方法进行增强

- execution是写明了需要增强的方法的位置（访问权限，返回值，全限定类名+方法的签名）

之后在增强的方法中通过反射获得对应的注解和注解属性值，进行对应的操作

具体实现：

- 在添加日志的时候，其实就是将该方法的相关信息插入到对应的数据库的表中，
- 在切入点使用注解的好处就是好的强耦合，如果使用execution方法，会加入很多不必要增强的方法，比如查看列表等操作，是无需存入日志的

- 为了添加操作的个性化，比如添加了进行了什么操作，处理的对象是什么。
  - 这就需要对每个方法都有一个特殊的值，而注解的属性刚好可以满足这个需求，
  - 只要在对应方法的注解中添加属性的值，之后在通知中通过反射获得注解中的属性值，就可以实现操作的个性化
- 反射的实现
  - 通过JoinPoint参数，可以获得需要增强的方法对象，以及方法所在的类，
    - 为什么需要获得方法所在的类 → 因为只能反射类，无法反射方法
  - 获得方法所在的类之后，通过反射可以得到对应的方法名，以及方法上是否存在自定义的注解
  - 之后通过遍历反射的方法，如果该方法的方法名和增强的方法名一致，执行插入数据库操作
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <dependencies>
        ...
        <!--数据校验，针对2.7.1，对于2.1.5REALSE无需导入 -->
        <dependency>
...
1. 验证码
	...
	综上，使用Redis更合理
        ...
        com.cskaoyan.controller.WxAuthController
...
使用AspectJ时用到了自定义注解，在元注解上添加在方法上使用和在运行时使用

并定义了多个属性，手动对这些属性赋值，为之后方法的增强做准备

在handler包中LogAspectHandler

## 反射

如果需要使用大量的get、set方法时，可以考虑反射；反射可以灵活的根据你传入的Class对象来调用对应的get，set方法

com.cskaoyan.controller.AdminConfigController
...
在计算优惠券结束日期时，在setTime方法里面使用了int进行计算，导致超出int最大值，设置了一个负毫秒数，最终得出的日期不正确，应使用long进行计算。
